...
public class SnakeGameWithoutTails {
    ...
    private static boolean nextDirection(String keyword) {
        ...
        if (board[x][y] == 2) {
            score++;
            board[x][y] = 0; // 아이템을 먹었으므로 해당 위치를 빈 타일로 설정
        }

        return true;
    }

    private static SnakeLocation move(int dx, int dy) {
        int newX = location.getX() + dx;
        int newY = location.getY() + dy;
        return new SnakeLocation(Math.max(0, Math.min(BOARD_SIZE - 1, newX)), Math.max(0, Math.min(BOARD_SIZE - 1, newY)));
    }
